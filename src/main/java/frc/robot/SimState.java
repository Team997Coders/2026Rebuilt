...
import static frc.robot.subsystems.Shooter.ShooterConstants.ROBOT_TO_SHOOTER_3D;
...
import frc.robot.util.sim.CustomRebuiltArena;
...
public class SimState {
  // TODO: find our intake extension treshhold (-30 is chickens number)
  ...
  // TODO: change actual height of shooter (5.875 is chickens number)
  private static final double SHOT_ORIGIN_HEIGHT_METERS =
      ROBOT_TO_SHOOTER_3D.getZ() + Units.inchesToMeters(5.875);

  private static final double BPS = 8.0; // not 8 :(
  ...
  private SimState() {
    ...
    var simConfig =
        DriveTrainSimulationConfig.Default()
            ...
            .withCustomModuleTranslations(DriveConstants.getModuleTranslations())
            .withBumperSize(
                Meters.of(Constants.BUMPER_LENGTH_X_METERS),
    ...
    new Trigger(efficiencyMode)
        .onChange(
            Commands.runOnce(() -> arena.setEfficiencyMode(efficiencyMode.get()))
  ...
  public void resetForAuto(Pose2d initialBluePose) {
    ...
    if (initialBluePose != null)
      RobotState.getInstance().resetPose(AllianceFlipUtil.apply(initialBluePose));
  }
  ...
  public void addSpindexerData(double spinVoltage) {
  ...
  public Pose2d getSimulatedTurretPose() {
    return getSimulatedRobotPose()
        .transformBy(
            new Transform2d(
                ROBOT_TO_SHOOTER_2D.getTranslation(), RobotState.getInstance().getTurretAngle()));
  }
  ...
  public Pose3d getSimulatedTurretPose3d() {
    return new Pose3d(getSimulatedRobotPose())
        .transformBy(
            new Transform3d(
                ROBOT_TO_SHOOTER_3D.getTranslation(),
                new Rotation3d(0, 0, RobotState.getInstance().getTurretAngle().getRadians())));
  }
  ...
  private void shootFuel() {
    ...
    Translation2d turretSpeedFromRobotRotation =
        new Translation2d(
            robotSpeeds.omegaRadiansPerSecond * ROBOT_TO_SHOOTER_2D.getTranslation().getNorm(),
            robotPose.getRotation().rotateBy(Rotation2d.kCW_90deg));
    ChassisSpeeds turretSpeeds =
        robotSpeeds.plus(
            new ChassisSpeeds(
    ...
    SimulatedArena.getInstance()
        .addGamePieceProjectile(
            new RebuiltFuelOnFly(
                robotPose.transformBy(ROBOT_TO_SHOOTER_2D).getTranslation(),
                Translation2d.kZero,
                turretSpeeds,
