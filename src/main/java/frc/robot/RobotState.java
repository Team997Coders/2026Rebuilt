...
public class RobotState {
  ...
  private final InterpolatingTreeMap<Double, Rotation2d> shuffleHoodAngleMap =
      new InterpolatingTreeMap<>(InverseInterpolator.forDouble(), Rotation2d::interpolate);
  private final InterpolatingDoubleTreeMap shuffleShooterSpeedMap =
  ...
  public Pose2d getTurretPose() {
    return getEstimatedPose()
        .transformBy(new Transform2d(ROBOT_TO_SHOOTER_2D.getTranslation(), lastTurretAngle));
  }
  ...
  public void addTurretObservation(
      Rotation2d turretAngle, double turretVelocityRadPerSec, double timestamp) {
    lastTurretAngle = turretAngle;
    lastTurretVelocityRadPerSec = turretVelocityRadPerSec;
    turretAngleBuffer.addSample(timestamp, turretAngle);
  }

  public boolean addVisionMeasurement(VisionIO.PoseObservation visionPose) {
  ...
  private boolean shouldAcceptVision(VisionIO.PoseObservation poseObservation) {
    Pose3d estimatedPose = poseObservation.pose();
    return poseObservation.tagCount() >= MIN_ACCEPTED_NUM_TAGS // Must see sufficient tags
        && (poseObservation.tagCount() > 1
            || poseObservation.ambiguity()
                < MAX_AMBIGUITY) // Must be multiple tags or low ambiguity
  ...
  private Matrix<N3, N1> getStdDevs(
      VisionIO.PoseObservation poseObservation, double xyCoeff, double rotCoeff) {
    double xyStdDev =
        xyCoeff * Math.pow(poseObservation.averageTagDistance(), 2.0) / poseObservation.tagCount();
    double rotStdDev =
        rotCoeff * Math.pow(poseObservation.averageTagDistance(), 2.0) / poseObservation.tagCount();
    return VecBuilder.fill(
        xyStdDev,
        xyStdDev,
        DriverStation.isEnabled() && poseObservation.tagCount() <= 1
  ...
  private AimingParameters getCompensatedAimingParameters(
    ...
    Pose2d turretPose = estimatedPose.transformBy(ROBOT_TO_SHOOTER_2D);
    ...
    Translation2d turretVelocity =
        new Translation2d(
            ...
            .plus(
                new Translation2d(
                    velocity.omegaRadiansPerSecond * ROBOT_TO_SHOOTER_2D.getTranslation().getNorm(),
    ...
    Logger.recordOutput(
        ...
        lookaheadPose.transformBy(ROBOT_TO_SHOOTER_2D.inverse()));
    Logger.recordOutput(
        "Aiming/" + loggingKey + "/CompensatedTarget",
        target.plus(turretPose.getTranslation().minus(lookaheadPose.getTranslation())));
    Logger.recordOutput(
        "Aiming/" + loggingKey + "/CompensatedTargetDistanceMeters", lookaheadDistance);
  ...
  public AimingParameters getHubAimingParameters() {
    ...
    lastHubAimingParameters =
        getCompensatedAimingParameters(
  ...
  public AimingParameters getShuffleAimingParameters() {
    ...
    lastShuffleAimingParameters =
        getCompensatedAimingParameters(
            shuffleTarget,
            distance ->
                shouldLowerHood() ? HoodConstants.MIN_ANGLE : shuffleHoodAngleMap.get(distance),
